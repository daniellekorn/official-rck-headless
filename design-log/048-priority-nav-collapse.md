...
## Addendum 2 — the real bug: the `>= 1280` branch never measured at all

The font-swap fix above shipped, but the bug was reported again, unchanged. It turned out to only be a secondary contributor — the actual root cause was the original design itself: `layout()`'s `window.innerWidth >= XL_BREAKPOINT` branch (the very first design decision above, item 1) never measures anything — it just trusts that the full item list plus Contact + Donate always fits at/above 1280px, because that's what 1280 was originally chosen to guarantee. That guarantee silently broke once `baseLinks` grew to 8 items (Torah Sheets was added later, in #053, well after this file's original design) — the row now genuinely doesn't fit in a real range just above 1280px, and the "trust CSS, don't measure" branch had nothing that could catch it.

Reproduced directly with a temporary headless-Chromium harness (`playwright-core`, installed with `--no-save` and removed after, same one-off pattern as design-log #053 addendum 16/17) driving the actual dev server at a sweep of widths. Against the previously-published code: at 1280/1300/1340px, `nav-pill.scrollWidth` (1342px) exceeded `clientWidth` by up to 106px, the Donate button's right edge landed ~100px past the pill's own right edge, and the hamburger toggle was computed `display: none` the whole time — the exact "spills out, no hamburger yet" symptom reported. At 1400px+ the row happens to fit again on its own, which is why the bug reads as a narrow band rather than "broken above 1280."

- **Fix**: removed the `>= XL_BREAKPOINT` early-return entirely (and the now-unused `XL_BREAKPOINT` constant). `layout()` now always forces everything visible, measures, and folds from the end — at *any* width, not just below 1280. The hamburger-visibility line changed from `removeProperty("display")` (defer to the `xl:hidden` class, which would hide it above 1280 even when folding just happened there) to an explicit `setProperty("display", "inline-flex")` — necessary once folding can happen at or above 1280, where the class would otherwise fight the fold.
- **Verification**: same headless-Chromium harness, re-run against the fixed code across 1180–1700px — `overflowing: false` and no Donate spillover at every width tested, hamburger correctly shown only in the 1280–1340px band that now needs folding and hidden everywhere else. This is a real before/after repro, not static review — the font-swap fix above is still correct and still needed (it's what makes a *narrower* real gap disappear too), but it was never sufficient on its own since the `>= 1280` branch it was patching around still didn't measure anything.
